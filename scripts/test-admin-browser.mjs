...
try {
  ...
  await page.locator('#reason').fill('sda');
  await page.locator('#ack').uncheck();
  await page.locator('#apply').click();
  assert.match(await page.locator('#action-message').innerText(), /Check the confirmation box/);
  assert.equal(posts.length, 0);
  await page.locator('#ack').check();
  let releaseDirectory, directoryStarted;
  const directoryRequested = new Promise(resolve => { directoryStarted = resolve; });
  heldRead = { started: directoryStarted, gate: new Promise(resolve => { releaseDirectory = resolve; }) };
  await page.evaluate(() => { queueLoad(1); });
  ...
  await page.locator('#apply').click();
  assert.match(await page.locator('#action-message').innerText(), /exact user ID/);
  await page.locator('#confirm').fill('user_test-student');
