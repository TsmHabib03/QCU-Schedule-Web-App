...
try {
  ...
  if (Number(health.meta?.adminMutationRecovery || 0) < 4) {
    failed = true;
    console.error('The published Apps Script is missing blocked-account recovery. Redeploy Code.gs from the current setup-database.gs.');
  } else console.log('Account deletion revision 4 (purge, purge_full, tombstone recovery) is deployed.');
} catch (_) {
  failed = true;
